...
public class Shooter {
    ...
    public static float shootingCoefficient = 2.705f;
    ...
    public void shoot() {
        ...
        if (shooting && !dumpShot) {
            ...
            if (Math.abs(calculatedAngle - tempAngle) > 0.004) {
                ...
                // if ((masterShooterMotor.getSelectedSensorVelocity() >
                // (shooterWheelLinearVelocityToMotorVelocity(calculatedVelocity)* 1.04585)) &&
                // setHoodYet) {
                double thresholdVelocity = (((((calculatedVelocity * shootingCoefficient * 11.666 - 500.0) * 1.0728)
                        * 1.00175) / 1.008) / 1.028);
    ...
    public void dumpShot() { // 92 inches according to distance method
        ...
        if (dumpShot && !shooting) {
            ...
            double thresholdVelocity = ((dumpShotSpeed * 22068.97) * 1.0045);
            System.out.println("calculated: " + thresholdVelocity);
            System.out.println("real:" + masterShooterMotor.getSelectedSensorVelocity(1));
            System.out.println(setHoodYet);
            if ((masterShooterMotor.getSelectedSensorVelocity(1) > thresholdVelocity) && setHoodYet) {
                if (DriverStation.isTeleop()) {
                    if (Constants.xbox.getRawAxis(3) > 0.5) {
                        Robot.intake.indexerMotor.set(-0.4);
